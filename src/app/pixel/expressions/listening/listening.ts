...
const LEFT_SOCKET = { x: 90, y: 80, width: 90, height: 65 };
const RIGHT_SOCKET = { x: 220, y: 80, width: 90, height: 65 };
const PUPIL_SIZE = { width: 22, height: 18 };
const PADDING = 8;
const BLINK_INTERVAL_MS = 15000;
...
export class Listening {
  ...
  protected isBlinking = signal(false);
  protected talking = signal(false);
  protected mouthOpen = signal(0);

  protected topControlY = computed(() => 215 - this.mouthOpen() * 40);
  protected bottomControlY = computed(() => 215 + this.mouthOpen() * 40);
  ...
  constructor(destroyRef: DestroyRef) {
    this.blinkIntervalId = setInterval(() => {
      ...
      this.blinkTimeoutId = setTimeout(() => this.isBlinking.set(false), BLINK_DURATION_MS);
    }, BLINK_INTERVAL_MS);
