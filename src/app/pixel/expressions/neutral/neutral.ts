...
@Component({
  ...
  styleUrl: './neutral.scss',
})
export class Neutral {
  protected talking = signal(false);
  protected mouthOpen = signal(0);

  protected topControlY = computed(() => 210 - this.mouthOpen() * 40);
  protected bottomControlY = computed(() => 210 + this.mouthOpen() * 40);
  protected showWrinkles = computed(() => this.mouthOpen() > WRINKLE_THRESHOLD);

  private lastPointerY = 0;
  private basePointerY = 0;

  /** Controla la apertura de la boca desde afuera (0 a 1), sincronizada con TTS. */
